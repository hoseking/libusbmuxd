...
PACKED(struct usbmuxd_result_msg{
	struct usbmuxd_header header;
	uint32_t result;
});

PACKED(struct usbmuxd_connect_request {
	struct usbmuxd_header header;
	uint32_t device_id;
...
PACKED(struct usbmuxd_listen_request {
	struct usbmuxd_header header;
});

PACKED(struct usbmuxd_device_record {
	uint32_t device_id;
	uint16_t product_id;
